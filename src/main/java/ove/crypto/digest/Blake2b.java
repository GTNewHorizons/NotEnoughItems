...
/*
 * A Java implementation of BLAKE2B cryptographic digest algorithm. Joubin Mohammad Houshyar <alphazero@sensesay.net>
 * bushwick, nyc 02-14-2014 -- To the extent possible under law, the author(s) have dedicated all copyright and related
 * and neighboring rights to this software to the public domain worldwide. This software is distributed without any
 * warranty. You should have received a copy of the CC0 Public Domain Dedication along with this software. If not, see
...
public interface Blake2b {
    ...
    public interface Spec {
        ...
        /**
         * max tree leaf length value.Note that this has uint32 semantics and thus 0xFFFFFFFF is used as max value
         * limit.
         */
        int max_tree_leaf_length = 0xFFFFFFFF;

        /**
         * max node offset value. Note that this has uint64 semantics and thus 0xFFFFFFFFFFFFFFFFL is used as max value
         * limit.
         */
    ...
    /**
     * A serializable / JSON-izable object usable for pausing a hash-in-process which can then be resumed with the same
     * Parameter the original digest was constructed with, and fed additional bytes to conclude the hash.
     */
    public static final class ResumeHandle implements Serializable {
        ...
        public Blake2b resume(Param param) {
            ...
            assert this.h != null && this.h.length == 8
                    ...
                    && this.m.length == 16
                    && this.v != null
                    && this.v.length == 16 : "Data is corrupted";
    ...
    static class Engine implements Blake2b {
        ...
        /*
         * impl note: these are not library funcs and used in hot loops, so no null or bounds checks are performed. For
    ...
    public static class Param implements AlgorithmParameterSpec {
        ...
        public final Param setPersonal(byte[] personal) {
            ...
            assert personal.length <= Spec.max_personalization_bytes : assertFail(
                    "personal.length",
                    personal.length,
                    inclusiveUpperBound,
